...
Source2:           %{pkg_name}-sentinel.service
Source3:           %{pkg_name}.service
Source4:           %{pkg_name}-sentinel.init
Source5:           %{pkg_name}.init
Source6:           %{pkg_name}-shutdown
Source7:           %{pkg_name}-limit-systemd
Source8:           %{pkg_name}-limit-init
Source9:           macros.%{pkg_name}
Source10:          https://github.com/antirez/%{pkg_name}-doc/archive/%{doc_commit}/%{pkg_name}-doc-%{short_doc_commit}.tar.gz
...
# Provides commented in collection
#Provides:          bundled(hiredis)
#Provides:          bundled(lua-libs)
#Provides:          bundled(linenoise)

%global redis_modules_abi 1
%global redis_modules_dir %{_libdir}/%{pkg_name}/modules
...
%prep
%setup -q -b 10 -n %{pkg_name}-%{version}
%setup -q -n %{pkg_name}-%{version}
mv ../%{pkg_name}-doc-%{doc_commit} doc
...
sed -i -e 's|^pidfile .*$|pidfile %{_root_localstatedir}/run/%{name}/redis.pid|g' redis.conf
sed -i -e '$ alogfile %{_localstatedir}/log/redis/sentinel.log' sentinel.conf
sed -i -e 's|^dir .*$|dir %{_localstatedir}/lib/redis|g' redis.conf
...
install -d %{buildroot}%{_localstatedir}/log/%{pkg_name}
install -d %{buildroot}%{_root_localstatedir}/run/%{name}
install -d %{buildroot}%{redis_modules_dir}

# Install logrotate file.
install -pDm644 %{S:1} %{buildroot}%{_root_sysconfdir}/logrotate.d/%{name}
sed -i -e 's|/var|%{_localstatedir}|g' %{buildroot}%{_root_sysconfdir}/logrotate.d/%{name}

# Install configuration files.
install -pDm640 %{pkg_name}.conf %{buildroot}%{_sysconfdir}/%{pkg_name}.conf
install -pDm640 sentinel.conf %{buildroot}%{_sysconfdir}/%{pkg_name}-sentinel.conf
...
sed -i -e 's|^ExecStart=.*$|ExecStart=%{_bindir}/redis-server %{_sysconfdir}/redis.conf --supervised systemd|g' %{buildroot}%{_unitdir}/%{name}.service
sed -i -e 's|^ExecStop=.*$|ExecStop=%{_libexecdir}/redis-shutdown|g' %{buildroot}%{_unitdir}/%{name}.service
sed -i -e 's|^ExecStart=.*$|ExecStart=%{_bindir}/redis-sentinel %{_sysconfdir}/redis-sentinel.conf --supervised systemd|g' %{buildroot}%{_unitdir}/%{name}-sentinel.service
sed -i -e 's|^ExecStop=.*$|ExecStop=%{_libexecdir}/redis-shutdown redis-sentinel|g' %{buildroot}%{_unitdir}/%{name}-sentinel.service

# Install systemd limit files (requires systemd >= 204)
install -p -D -m 644 %{S:7} %{buildroot}%{_root_sysconfdir}/systemd/system/%{name}.service.d/limit.conf
install -p -D -m 644 %{S:7} %{buildroot}%{_root_sysconfdir}/systemd/system/%{name}-sentinel.service.d/limit.conf
%else # install SysV service files
install -pDm755 %{S:4} %{buildroot}%{_root_initrddir}/%{name}-sentinel
install -pDm755 %{S:5} %{buildroot}%{_root_initrddir}/%{name}
sed -i -e 's|^exec=.*|exec="%{_bindir}/\$name"|g' %{buildroot}%{_root_initrddir}/%{name}
sed -i -e 's|^shut=.*|shut="%{_libexecdir}/redis-shutdown"|g' %{buildroot}%{_root_initrddir}/%{name}
sed -i -e 's|^pidfile=.*|pidfile="%{_root_localstatedir}/run/%{name}/redis.pid"|g' %{buildroot}%{_root_initrddir}/%{name}
sed -i -e 's|^SENTINEL_CONFIG=.*|SENTINEL_CONFIG="%{_sysconfigdir}/redis.conf"|g' %{buildroot}%{_root_initrddir}/%{name}
sed -i -e 's|/etc/sysconfig/redis|%{_sysconfdir}/sysconfig/redis"|g' %{buildroot}%{_root_initrddir}/%{name}
sed -i -e 's|^exec=.*|exec="%{_bindir}/\$name"|g' %{buildroot}%{_root_initrddir}/%{name}-sentinel
sed -i -e 's|^shut=.*|shut="%{_libexecdir}/redis-shutdown"|g' %{buildroot}%{_root_initrddir}/%{name}-sentinel
sed -i -e 's|^pidfile=.*|pidfile="%{_root_localstatedir}/run/%{name}/sentinel.pid"|g' %{buildroot}%{_root_initrddir}/%{name}-sentinel
sed -i -e 's|^SENTINEL_CONFIG=.*|SENTINEL_CONFIG="%{_sysconfigdir}/redis-sentinel.conf"|g' %{buildroot}%{_root_initrddir}/%{name}
sed -i -e 's|/etc/sysconfig/redis-sentinel|%{_sysconfdir}/sysconfig/redis-sentinel|g' %{buildroot}%{_root_initrddir}/%{name}-sentinel


# Install SysV limit files
install -p -D -m 644 %{S:8} %{buildroot}%{_sysconfdir}/security/limits.d/95-%{name}.conf
#what about limit sentinel?
%endif
...
install -pDm755 %{S:6} %{buildroot}%{_libexecdir}/%{pkg_name}-shutdown

# Install redis module header
install -pDm644 src/%{pkg_name}module.h %{buildroot}%{_includedir}/%{pkg_name}module.h

%if 0%{?with_redistrib}
# Install redis-trib
install -pDm755 src/%{pkg_name}-trib.rb %{buildroot}%{_bindir}/%{pkg_name}-trib
%endif
...
doc=$(echo %{buildroot}/%{_docdir}/%{pkg_name})
...
%pre
getent group %{pkg_name} &> /dev/null || \
groupadd -r %{pkg_name} &> /dev/null
getent passwd %{pkg_name} &> /dev/null || \
useradd -r -g %{pkg_name} -d %{_sharedstatedir}/%{pkg_name} -s /sbin/nologin \
-c 'Redis Database Server' %{pkg_name} &> /dev/null
exit 0
...
%dir %attr(0750, redis, redis) %{_libdir}/%{pkg_name}
%dir %attr(0750, redis, redis) %{redis_modules_dir}
%dir %attr(0750, redis, redis) %{_sharedstatedir}/%{pkg_name}
%dir %attr(0750, redis, redis) %{_localstatedir}/log/%{pkg_name}
%if 0%{?with_redistrib}
%exclude %{_bindir}/%{pkg_name}-trib
%endif
%exclude %{macrosdir}
%exclude %{_includedir}
%exclude %{_docdir}/%{pkg_name}/*
%{_bindir}/%{pkg_name}-*
...
%dir %attr(0755, redis, redis) %ghost %{_root_localstatedir}/run/%{name}
%else
%{_initrddir}/%{name}
%{_initrddir}/%{name}-sentinel
%config(noreplace) %{_root_sysconfdir}/security/limits.d/95-%{name}.conf
%dir %attr(0755, redis, redis) %{_root_localstatedir}/run/%{name}
%endif

%files devel
%license COPYING
%{_includedir}/%{pkg_name}module.h
%{macrosdir}/*
...
%files trib
%license COPYING
%{_bindir}/%{pkg_name}-trib
%endif


%changelog
* Tue Jan  9 2018 Jakub Janco <jjanco@redhat.com> - 4.0.6-2
- make SPEC collection compatible
